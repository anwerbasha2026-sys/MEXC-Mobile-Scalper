...
import os, time, sqlite3, hmac, hashlib, threading
...
def sell_market(symbol, api, secret):
    try:
        ...
        if qty <= 0: return True, "لا يوجد رصيد للبيع"
...
def ema_trend(symbol, interval):
    ...
    data=safe_get(BASE_URL+"/klines",{"symbol":symbol,"interval":interval,"limit":300})
...
def check_conditions(symbol):
    try:
        ...
        data=safe_get(BASE_URL+"/klines",{"symbol":s,"interval":"5m","limit":300})
        if not data or len(data)<200: return False,0,"بيانات الشموع غير كافية"
...
class MEXCMobileApp(App):
    ...
    def values(self):
        ...
        api=ids.api.text.strip(); secret=ids.secret.text.strip()
        try: amount=float(ids.amount.text); tp=float(ids.tp.text); sl=float(ids.sl.text)
        except: amount,tp,sl=109,1.5,2
    ...
    def start_ticker(self):
        if self.ticker_thread and self.ticker_thread.is_alive(): return
        self.ticker_thread=threading.Thread(target=self.ticker_loop,daemon=True)
        self.ticker_thread.start()

    def ticker_loop(self):
        while self.position:
            p=get_price(self.position["symbol"])
            if p and self.position["entry_price"]>0:
                pct=(p-self.position["entry_price"])/self.position["entry_price"]*100
                usd=self.position["amount"]*pct/100
                Clock.schedule_once(lambda dt,p=p,u=usd,x=pct:self.update_pnl(p,u,x),0)
                if pct>=self.position["tp_percent"]:
                    Clock.schedule_once(lambda dt:self.auto_close("TP"),0); return
                if pct<=-self.position["sl_percent"]:
                    Clock.schedule_once(lambda dt:self.auto_close("SL"),0); return
            time.sleep(1)
    ...
    def close_position(self,reason="MANUAL"):
        ...
        if reason!="MANUAL": self.start_scanning()
    ...
    def save_inputs(self):
        api,secret,amount,tp,sl=self.values()
        save_setting("api_key",api); save_setting("secret_key",secret)
        save_setting("amount",amount); save_setting("tp",tp); save_setting("sl",sl)
